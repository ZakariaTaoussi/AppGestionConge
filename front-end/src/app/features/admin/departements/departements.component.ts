...
export class AdminDepartementsComponent {
  ...
  enregistrer(): void {
    ...
    if (this.editingId) {
      ...
      if (departement) Object.assign(departement, this.form);
    } else {
      this.departements = [...this.departements, { ...this.form, id: Date.now() }];
    }
    this.reinitialiser();
  }
  ...
  modifier(departement: Departement): void {
    ...
    this.form = { nom: departement.nom, responsable: departement.responsable, effectif: departement.effectif };
  }
  ...
  private emptyForm(): Omit<Departement, 'id'> {
    return { nom: '', responsable: '', effectif: 0 };
  }
}
